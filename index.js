...
bot.on("ready", () => console.log(`[Namang] ${bot.user.tag}`));
bot.on("ready", () =>
...
bot.on("message", async (message) => {
    ...
    if (command === "한강" | command === "한강물" | command === "한강온도" | command === "한강수온" | command === "한강물온도") {
        ...
        const say = function () {
            if (body.WPOSInformationTime.row[saya].W_TEMP === "점검중으로 표시할 수 없어요.") {
              saya ++;
              say();
            } else {
              goo();
        ...
        function goo () {
            ...
            .setDescription(side + "구역의 " + msda.slice(4, 6) + "월 " + msda.slice(6, 8) + "일 " + msti[0] + "시 " + ' 측정값 이에요.' )
            .setThumbnail("https://cdn.discordapp.com/attachments/767280811220926504/799066007095607296/alice_logo_gif.gif")
            .setFooter("byNamang407 / 자료제공 : 서울시열린데이터광장");
